...
class NotebookGraph(ast.NodeVisitor):
    ...
    def build_graph(self):
        ...
        # #print(self.defed_functions)
        self.visit(self.root_node)
        self.nodes_sorted()
        self.point_dataset_node()
        self.point_model_node()
        # #print(self.data_varibles_to_current_node.keys())
        # #print(type(self.root_node).__name__)
        ...
        while node_index < len(self.nodes):
            ...
            if self.nodes[node_index].is_dataset_node == True:
                for index,child in enumerate(self.nodes[node_index].childrens):
                    # child.is_dataset_node = True
                    # #print('/////')
                    
                    for childinx , childn in enumerate(self.nodes):
                        # #print("childn", childn.sorted_id)
                        # #print('child', child.sorted_id)
                        if childn.sorted_id == child.sorted_id:
                            self.nodes[childinx].is_dataset_node=True
                            # #print(childinx)
                            # #print('child node true')
                    for childeinx, childe in enumerate(self.edges):
                        if childe.edge_id == self.nodes[node_index].children_edges[index].edge_id:
                            self.edges[childeinx].is_dataset_edge = True
                            # #print(childeinx)
                            # #print('child edge true')
                    self.nodes[node_index].children_edges[index].is_dataset_edge = True
    ...
    def delete_nodes(self):
        ...
        for node in self.nodes:
            found_index = 0
            # #print('node.is_model_node',node.is_model_node)
            if node.is_model_node == True:
                # #print("model node", node.sorted_id)
                continue
            is_in = False
    ...
    def point_model_node(self):
        ...
        #print('self.model_variable', self.model_variable)
        node_index = 0
        while node_index < len(self.nodes):
            #print('self.nodes[node_index].varible_name', self.nodes[node_index].varible_name)
        ...
        # while node_index < len(self.nodes):
        #     for index,child in enumerate(self.nodes[node_index].childrens):
        #         if child.is_model_node == True and self.nodes[node_index].children_edges[index].edge_type == 2:
        #             self.nodes
        #         self.nodes[node_index].children_edges[index].is_model_edge = True
        #         self.nodes[node_index].childrens[index].is_model_node =True
            ...
            for edge_index,edge in enumerate(self.result_nodes[index].children_edges):
                if self.result_nodes[index].children_edges[edge_index].edge_type == 1: # white
                    ...
                    for line_index,line in enumerate(code_list):
                        ...
                        # #print(self.result_nodes[index].children_edges[edge_index].original_code[0:-1] in line)
                        #print("xxxx")
                        #print('line', line)
                        #print(self.result_nodes[index].children_edges[edge_index].original_code[0:-1])
                        ...
                        # #print(replaced_line4)
                        # if self.result_nodes[index].children_edges[edge_index].original_code[0:-1] == "df.drop(['PatientId', 'AppointmentID', 'ScheduledDay', 'Handcap', 'AppointmentDay', 'Neighbourhood'], axis=1, inplace=True)":
                        #     #print("xxxxxx")
                        #     #print(self.result_nodes[index].children_edges[edge_index].original_code[0:-1])
                        #     #print('line', line)
                        # #print(line)
                        if self.result_nodes[index].children_edges[edge_index].original_code[0:-1] in line or replaced_line2 in line or replaced_line3 in line or replaced_line4 in line or replaced_line3 in replace_line or replaced_line4 in replace_line:
                            ...
                            # self.result_edges[found_edge_index].line_id.append(line_index)
                            #print("##############################")
                            #print("line_index", line_index)
                            #print('line',line,'edge', self.result_nodes[index].children_edges[edge_index].original_code)
                            #print("line_id", self.result_nodes[index].children_edges[edge_index].line_id)
                            #print(self.result_nodes[index].children_edges[edge_index].line_id)
                            #print(self.result_edges[found_edge_index].line_id)
...
# nx.draw_networkx_nodes(G, pos)
    ...
    def get_variable_from_node(self, node): # [(varible_name, is_subscript)]
        ...
        if type(node).__name__ == 'list':
            ...
            for i in result:
                ...
                new_result.append(i)
            return new_result
    def get_variable_from_node_insub(self, node): # [(varible_name, is_subscript)]
        # #print(type(node).__name__)
        if type(node).__name__ == 'list':
            result = []
            for one_node in node:
            ...
            self.start_get_varible = True
            self.visit(node)
            #print('self.get_varible_results', self.get_varible_results)
            result = self.get_varible_results
            self.get_varible_results = []
            self.start_get_varible = False
            self.start_get_varible_insub = False
            ...
            for i in result:
                ...
                new_result.append(i)
            return new_result
    def get_variable_from_node_with_sub(self, node): # [(varible_name, is_subscript)]
        # #print(type(node).__name__)
        if type(node).__name__ == 'list':
            result = []
            for one_node in node:
    ...
    def found_func_name(self,node):
        if type(node) == ast.Name:
            ...
            # #print('Attribute', astunparse.unparse(node))
    ...
    def create_black_edge(self, node, graph_node=None, one_target_name=None, targets = None, varibles=None):
        #print("    ###########")
        #print('    create black edge')
        
        # #print('    black varibles:', varibles)
        #print('    black code:', astunparse.unparse(node))
        # if varibles is None:
            # #print(type(node).__name__)
            # sys.exit()
        if varibles==None:
        ...
        if graph_node is not None:
            if one_target_name is None:
                ...
                self.sorted_node_id += 1
            self.nodes.append(new_node)
            # #print(graph_node.varible_name, self.data_varibles_to_current_node[graph_node.varible_name])
            new_edge = Edge(self.edge_id, self.sorted_edge_id,'-BLACK-', astunparse.unparse(node), 0)
            self.sorted_edge_id += 1
            self.edge_id += 1
    ...
    def create_white_edge(self, node, func_name):
        ...
        #print("    ###########")
        #print('    create white edge')
        # #print('    white varibles:', varibles)
        #print('    white code:', astunparse.unparse(node))
        
        # if 
        # if len(varibles) == 0:
        #     varibles = self.get_variable_from_node(node)
        #     varibles = [item for item in varibles if item in self.data_varibles_to_current_node.keys()]
        ...
        #print('    white varibles:', varibles)
        #print(self.data_varibles_to_current_node.keys())
        # for item in varibles:
        #     if item not in varibles:
        #         varibles.append(item)


            # #print('white func_varibles', func_varibles)
            # if len(func_varibles) == 1:
            #     new_node = Node(self.sorted_node_id, func_varibles[0])
            #     self.sorted_node_id += 1
                
            #     varible_node = self.data_varibles_to_current_node[func_varibles[0]]
            #     # #print("func edge", func_name)
    ...
    def create_assign_edge(self, target_varibles, node, graph_node=None):
        #print("    ###########")
        #print('    create assign edge')
        #print('    assign target:', target_varibles)
        #print('    assign code:', astunparse.unparse(node))
        if graph_node is not None:
    ...
    def visit_Expr(self, node): # is a stmt
        if self.start_get_varible:
            ...
            if isinstance(node.value, ast.Call):
                ...
                self.should_create_white_edge = False
                self.should_create_black_edge = True
            #print('self.should_create_white_edge', self.should_create_white_edge)
            self.visit(node.value)
            if self.white_edge_node is not None:
                #print('Expr white_edge_node', self.white_edge_node)
                ...
                self.white_edge_node = None
            #print('should_create_black_edge', self.should_create_black_edge)
    ...
    def assign_func(self, node, assign_type = 1):
        if self.start_get_varible:
            ...
            if self.start_get_varible_flow:
                ...
                #print('\033[1;33;44m' + str(set(value_varibles1) - set(value_varibles))+ '\033[0m')
                # #print('value_varibles', value_varibles)
                # #print('node.value', type(node.value).__name__)
                # target_varibles = self.get_variable_from_node(node.targets)
                ...
                if len(value_varibles) == 0:
                    ...
                    if isinstance(node.value, ast.Call):
                        ...
                        if self.white_edge_node is not None:
                            # #print('assign self.white_edge_node', self.white_edge_node)
                            self.nodes.append(self.white_edge_node)
                            self.data_varibles_to_current_node[self.white_edge_node.varible_name] = self.white_edge_node
                        ...
                        # #print(self.data_varibles_to_current_node)
...
def save_graph(filepath):
    ...
    try:
        with open('HybridPipeGen/core/tmpdata/prenotebook_graph/'+filename+".pkl", 'wb') as f:
            pickle.dump(graph, f)
    except:
        pass
...
def count_edge(is_dataset):
    root_path = 'graphs/'
    filenames = os.listdir(root_path)
    all_edge_dict = {}
    all_split_point_edge_dict = {}
    count = 0
    for file_name in filenames:
        ...
        with open(file_path, 'rb') as f:
            ...
            edges = graph.edges
            for edge in edges:
                if is_dataset and not edge.is_dataset_edge:
                    continue
                if edge.func_name not in all_edge_dict:
                    all_edge_dict[edge.func_name] = 0
                all_edge_dict[edge.func_name] += 1
                if edge.func_name.split(".")[-1] not in all_split_point_edge_dict:
                    all_split_point_edge_dict[edge.func_name.split(".")[-1]] = 0
                all_split_point_edge_dict[edge.func_name.split(".")[-1]] += 1
    # #print(count)
    # pprint.p#print(all_edge_dict)
    if is_dataset:
        with open("all_edge_dict_is_dataset.json", 'w') as f:
            json.dump(all_edge_dict, f)
        with open("all_edge_split_point_dict_is_dataset.json", 'w') as f:
            json.dump(all_split_point_edge_dict, f)
    else:
        with open("all_edge_dict_not_dataset.json", 'w') as f:
            json.dump(all_edge_dict, f)
...
if __name__ == "__main__":
    ...
    # count_edge(True)
    # graph = NotebookGraph()
    # graph.load_file("prenotebook_code/1407110.py")
    # graph.build_graph()
    # graph.layout(directory="rmmodel_figs")
    # graph.delete_nodes()
    # graph.update_edges()
    # graph.get_white_edge_line()
    # graph.layout(directory="prenotebook_resfigs/", all_nodes=False)
    # # # #print(filename)
    # with open('prenotebook_graph/1407110.pkl', 'wb') as f:
        # pickle.dump(graph, f)
    # graph.layout(directory="res_figs", all_nodes=False)

    
    # for node in graph.nodes:
    #     #print('sorted_id', node.sorted_id)
    # graph.show_dataset_graph()
    # # graph.look_edges()
    # # graph.look_nodes()
    # graph.layout()
    # count_edge()
