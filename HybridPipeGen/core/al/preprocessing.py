...
def generate_hi_seq(notebook_id, traintest):
    if traintest == 'test':
        json_path = config.param_candidate_hybrid_val_json_save_root_path
    else:
        json_path = config.param_candidate_offline_hybrid_val_json_save_root_path
    edge_ids = []
...
def batch_generate_features(traintest, thread_id, thread_num):
    if traintest == 'test':
        notebook_path = config.param_candidate_hybrid_val_json_save_root_path
        save_path = config.test_features
    else:
        notebook_path = config.param_candidate_offline_hybrid_val_json_save_root_path
...
def generate_training_data():
    notebook_path = param_candidate_offline_hybrid_val_json_save_root_path
    save_path = config.train_features
    ...
    for index,notebook_id in enumerate(notebooks):
        ...
        for index, seq_id in enumerate(features['seq_ids']):
            sv_path = config.param_offline_hybrid_val_result_save_root_path + notebook_id + '/' + seq_id + '.npy'
...
if __name__ == '__main__':
    ...
    # para_gen_features('train')
    para_gen_features('test')
    
    # generate_training_data()
    # move_test_train()
    # batch_generate_features('train', 1,1)
    # batch_generate_features('test',1,1)
    # notebook_id = 'aadhavvignesh_practical-ml-2-logistic-regression'
    # dataset_features, pipeline_features, seq_ids = generate_dataset_pipeline_features(notebook_id)
