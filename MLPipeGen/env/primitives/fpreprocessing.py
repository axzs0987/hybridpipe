...
class MinMaxScalerPrim(Primitive):
    ...
    # def fit(self, data):
    #     cat_train_x, num_train_x = catch_num(data)
    #     num_train_x = deepcopy(num_train_x)
    ...
    #     data_x = pd.concat([cat_train_x.reset_index(drop=True), num_train_x.reset_index(drop=True)],axis=1)
    #     return data_x
...
class KBinsDiscretizerOrdinalPrim(Primitive):
    ...
    def can_accept(self, data):
        ...
        #     if np.unique(data[c]).shape[0] < 10:
        #         return False
        return True
...
#         cat_train_x, num_train_x = catch_num(data)
#         num_train_x = deepcopy(num_train_x)
#         self.preprocess = ColumnTransformer([("discrit", KBinsDiscretizer(), list(num_train_x.columns))])
#         self.preprocess.fit(num_train_x)

#         num_train_x = pd.DataFrame(self.preprocess.transform(num_train_x), columns=list(num_train_x.columns))
#         data_x = pd.concat([cat_train_x.reset_index(drop=True), num_train_x.reset_index(drop=True)],axis=1)
#         return data_x
