...
class Pipeline:
    ...
    def add_step(self, step): # step is a Primitive
        ...
        if step.type in pre_pipeline or not step.can_accept(self.train_x) or not step.can_accept(self.test_x) or (not step.is_needed(self.train_x) and not step.is_needed(self.test_x)):
            print('step.type in pre_pipeline', step.type in pre_pipeline)
            print('not step.can_accept(self.train_x)', not step.can_accept(self.train_x))
            print('not step.can_accept(self.test_x)', not step.can_accept(self.test_x))
            print('not step.is_needed(self.train_x)', not step.is_needed(self.train_x))
            print('not step.is_needed(self.test_x)', not step.is_needed(self.test_x))
            print('bad step')
            return 0
        
        try:
            # print('111')
            if type(self.train_x).__name__ == 'DataFrame':
                print('self.train_x before', self.train_x)
            # if type(self.test_x).__name__ == 'DataFrame':
                # print('self.test before', self.test_x.isnull().sum())
            self.train_x, self.test_x = step.transform(self.train_x, self.test_x, self.train_y)
            # print('222')
            # print('self.train_x', self.train_x)
            # print('type(self.train_x).__name__', type(self.train_x).__name__)
            if type(self.train_x).__name__ == 'DataFrame':
                # print('self.train_x.discribe', self.train_x.describe())
                print('self.train_x', self.train_x)
            # print('self.test_x', self.test_x)
            # print('type(self.test_x).__name__', type(self.test_x).__name__)
            # if type(self.test_x).__name__ == 'DataFrame':
                # print('self.test_x.describe', self.test_x.describe())
                # print('self.test_x.isnull', self.test_x.isnull().sum())
            ...
            print('self.train_x.columns', self.train_x.columns)
            print('self.cat_cols', self.cat_cols)
            # print('self.num_cols', self.num_cols)
            # print('self.cat_cols', self.cat_cols)
                # self.train_x.to_csv('train_x_'+str(self.index)+'_test.csv')
                # self.test_x.to_csv('test_x_'+str(self.index)+'_test.csv')
